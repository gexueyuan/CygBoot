...
/**
  ...
  *         1: error occurred
  */
uint32_t FLASH_If_Erase(uint32_t StartAddress,uint32_t FlashSize)
{
  uint32_t UserStartSector,UserEndSector, i = 0;

  /* Get the sector where start the user flash area */
  UserStartSector = GetSector(StartAddress);
  UserEndSector =  GetSector(StartAddress+FlashSize);
  ...
  for (i = 0; (i < DataLength); i++)
  {
    ...
    if (FLASH_ProgramWord(*FlashAddress, *(uint32_t*)(Data+i)) == FLASH_COMPLETE)
    {
     ...
     /* Check the written value */
      ...
      {
        printf("flash write not match:write data:%x,flash data:%x",*(uint32_t*)(Data+i),*(uint32_t*)*FlashAddress);
...
{
  uint32_t UserStartSector = FLASH_Sector_5;
...
{
  __IO uint32_t UserStartSector = FLASH_Sector_1, UserWrpSectors = OB_WRP_Sector_5;
